...
void BinByBinFactory::AddBinByBin(CombineHarvester &src, CombineHarvester &dest) {
  ...
  for (unsigned i = 0; i < procs.size(); ++i) {
	...
	// now correlate the stat unc between DCPp and DCPm
        if (
	    sys.process()=="embedded" or sys.process()=="W" or sys.process()=="VVT" or sys.process()=="TTT" or sys.process()=="ZLL" or sys.process()=="TT" or sys.process()=="VV" or sys.process()=="EWK" or sys.process()=="ZJ" or sys.process()=="QCD" or sys.process()=="ZL" or sys.process()=="TTL" or sys.process()=="VVL" or sys.process()=="jetFakes" or sys.process()=="GGH2Jets_sm_M" or sys.process()=="GGH2Jets_pseudoscalar_M" or sys.process()=="reweighted_WH_htt_0PM" or sys.process()=="reweighted_ZH_htt_0PM" or sys.process()=="reweighted_qqH_htt_0PM" or sys.process()=="reweighted_qqH_htt_0M" or sys.process()=="reweighted_ZH_htt_0M" or sys.process()=="reweighted_WH_htt_0M" or sys.process()=="ggH_hww125" or sys.process()=="qqH_hww125" or sys.process()=="VH_hww125"
	    )	  
	  {
          boost::replace_all(name, "_DCPp", "");
          boost::replace_all(name, "_DCPm", "");
        }
