...
for mycat in categ:
    ...
    print "getting TotalBkg from %s %s_prefit"%(myfile.GetName(),mycat)
    B=myfile.Get(mycat+"_prefit").Get("TotalBkg")
    ZTT=myfile.Get(mycat+"_prefit").Get("ZTT")
    if args.isEmbed=="1":
        ZTT=myfile.Get(mycat+"_prefit").Get("embedded")
    ZL=myfile.Get(mycat+"_prefit").Get("ZL")
    if myfile.Get(mycat+"_prefit").Get("ZJ") and ZL:
       ZL.Add(myfile.Get(mycat+"_prefit").Get("ZJ"))
    if not ZL:
        if args.channel=="em":
            #ZL=myfile.Get(mycat+"_prefit").Get("ZLL")
            ZL=myfile.Get(mycat+"_prefit").Get("ZL")
        else:
            ZL=myfile.Get(mycat+"_prefit").Get("ZJ")
    TT=myfile.Get(mycat+"_prefit").Get("TT")
    if not TT and args.isEmbed=="0":
      print " => adding TTT and TTL, TTJ"
      TT=myfile.Get(mycat+"_prefit").Get("TTT")
      if myfile.Get(mycat+"_prefit").Get("TTL"):
        TT.Add(myfile.Get(mycat+"_prefit").Get("TTL"))
      if myfile.Get(mycat+"_prefit").Get("TTJ"):
        TT.Add(myfile.Get(mycat+"_prefit").Get("TTJ"))
    elif args.channel=="em":
      print " => adding TTL"
      TT=myfile.Get(mycat+"_prefit").Get("TTL")
    
    VV=myfile.Get(mycat+"_prefit").Get("VV")
    if not VV and args.isEmbed=="0":
      VV=myfile.Get(mycat+"_prefit").Get("VVT")
      if myfile.Get(mycat+"_prefit").Get("VVJ"):
        VV.Add(myfile.Get(mycat+"_prefit").Get("VVJ"))
    EWKZ=myfile.Get(mycat+"_prefit").Get("EWKZ")
    HWW=myfile.Get(mycat+"_prefit").Get("ggH_hww125")
    if args.isEmbed=="1" and args.channel=="tt":
      TT=myfile.Get(mycat+"_prefit").Get("TTL")
      print " getting VVL..."
      VV=myfile.Get(mycat+"_prefit").Get("VVL")
    ...
    if HWW:
       HWW.Add(myfile.Get(mycat+"_prefit").Get("qqH_hww125"))
    W=myfile.Get(mycat+"_prefit").Get("W")
    QCD=myfile.Get(mycat+"_prefit").Get("jetFakes")
    if args.channel=="em":
        QCD=myfile.Get(mycat+"_prefit").Get("QCD")
    S=myfile.Get(mycat+"_prefit").Get("TotalSig")
    D=myfile.Get(mycat+"_prefit").Get("data_obs")
    factor=0
    if "_3_" in mycat:
      factor=1
    elif "_4_" in mycat:
...
c.Modified()
c.SaveAs("plots/unrolled_"+args.variable+"_"+args.channel+"_"+args.year+"_prefit.pdf")
c.SaveAs("plots/unrolled_"+args.variable+"_"+args.channel+"_"+args.year+"_prefit.png")
