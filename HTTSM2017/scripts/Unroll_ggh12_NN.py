...
processes = [
    'data_obs', 'embedded', 'VVT', 'W', 'ZL', 'ZJ', 'TTT', 'TTJ', 'VVJ', 'ggH125', 'VBF125',
    'WH125', 'ZH125', 'jetFakes', 'reweighted_qqH_htt_0L1125', 'reweighted_qqH_htt_0L1Zg125',
...
for i in range (0,ncat): # loop over categories
    ...
    for i_histo in processes: # loop over input histos (processes)
        ...
        for systematic in systematics : # loop over available systematics
            if file.Get(categories[i]).Get(i_histo+systematic+'Down') != None or file.Get(categories[i]).Get(i_histo+systematic+'down'):
                print categories[i]+'/'+i_histo+systematic+'Down/Up'
                if '_ff_' in systematic:
                  ...
                  histo_d.SetName(histo2D_d.GetName().replace('_up', '_Up')) #
                  histo_u.SetName(histo2D_u.GetName().replace('_down', '_Down')) #
                else:
                  histo2D_d=file.Get(categories[i]).Get(i_histo+systematic+'Down') #
                  histo2D_u=file.Get(categories[i]).Get(i_histo+systematic+'Up') #
                  histo_d=ROOT.TH1F('histo_d',histo2D_d.GetName(),nx*ny,0,nx*ny) #
                ...
                for j in range(1,nx+1):
                    for k in range(1,ny+1):
                        ...
                        cont_d = histo2D_d.GetBinContent(n)
                        if cont_u < 0:
